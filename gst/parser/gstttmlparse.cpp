    ...
    gsize* consumed)
{
  ...
  *consumed = 0;
  ...
  GST_LOG_OBJECT (self,
      "successfully converted %" G_GSIZE_FORMAT
      " characters from %s to UTF-8"
      "%s",
      len, encoding, (err) ? " , using ISO-8859-15 as fallback" : "");
...
parse_mdvdsub (ParserState* state, const gchar* line)
{
  ...
  if (sscanf (line, "{%u}{%u}", &start_frame, &end_frame) != 2) {
    g_warning (
        "Parse of the following line, assumed to be in microdvd .sub"
        " format, failed:\n%s",
        line);
    return NULL;
  }
...
parse_lrc (ParserState* state, const gchar* line)
{
  ...
  state->start_time = gst_util_uint64_scale (m, 60 * GST_SECOND, 1) + gst_util_uint64_scale (s, GST_SECOND, 1) + gst_util_uint64_scale (c, milli * GST_MSECOND, 1);
...
parse_mpsub (ParserState* state, const gchar* line)
{
  ...
  switch (state->state) {
    case 0:
      ...
      return NULL;
    case 1: { /* No need to parse that text if it's out of segment */
      guint64 clip_start = 0, clip_stop = 0;
      gboolean in_seg = FALSE;
